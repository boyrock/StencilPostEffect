...
Shader "mattatz/MaskBloom" {
	...
	SubShader {
		...
		half4 fragScreen(v2f_img i) : SV_Target{
			half4 screencolor = tex2D(_MainTex, i.uv);
			half4 addedbloom = tex2D(_BloomTexture, i.uv);
			half4 result = 1 - (1 - addedbloom * _Intensity) * (1 - screencolor);
			return result;// mask(screencolor, result);
		}

		half4 fragAdd(v2f_img i) : SV_Target{
			half4 screencolor = tex2D(_MainTex, i.uv);
			half4 addedbloom = tex2D(_BloomTexture, i.uv);
